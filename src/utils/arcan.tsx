...
export function settlementDateParser(data: string): IPreparedDate | false {
  ...
  const d = date.length < 2 ? date : date[0] === '0' ? date[1] : date;
  const m = month.length < 2 ? month : month[0] === '0' ? month[1] : month;
  if (+d > 31 || +m > 12) {
    return false;
  }
...
function positionPreparing(val: number, key: number | string): IPosition {
  ...
  const type = mains.findIndex((n) => n === key) >= 0 ? EArcaneType.main : karmics.findIndex((n) => n === key) >= 0
    ? EArcaneType.karmic
    : shadows.findIndex((n) => n === key) >= 0 ? EArcaneType.shadow : null;
